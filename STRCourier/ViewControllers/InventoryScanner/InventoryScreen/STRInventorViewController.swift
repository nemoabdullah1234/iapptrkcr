...
class STRInventorViewController: UIViewController,UITabBarDelegate,UITableViewDataSource,CLLocationManagerDelegate,UIAlertViewDelegate, LocationSelectorDelegate {
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        vw.frame =  CGRect(x: 0, y: 0, width: tableView.frame.size.width, height: 60)
          ...
          let viewController = STRInventorViewController_copy.init(nibName:"STRInventorViewController_copy", bundle: nil)
           let dict = self.zone_obj[section]
                  utility.setselectedFloor(dict)
               viewController.dictFloorJT = dict
                viewController.LocationName = self.lblAddress.text
...
//            utility.setflagSession(false)
//            //show alert
//            let str = "We have detected your location as \(String(describing: arr!.first!["address"])). Do you want to switch?"
...
//            self.currentID = dict.first!["locationId"] as? String
//            self.tblInventory.reloadData()
//            if(self.zone_obj.count == 0){
//                self.addNodata()
//            }
...
//        else{
            //show search :D
        if(appearInformation != .strFromItemDetail && appearInformation != .strFromFirstLoadPersisted)//new added if
        {
            appearInformation = .strFromSearchLocation
            let vw  = STRChangeLocationViewController(nibName: "STRChangeLocationViewController", bundle: nil)
            vw.delegate = self
...
extension STRInventorViewController : KTKBeaconManagerDelegate
{
